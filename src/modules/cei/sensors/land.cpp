...
void Cei::landInit()
{
	...
	if (_landCount > REQ_LAND_INIT_COUNT) {
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] land init");
		_sensorTimeout &= ~SENSOR_LAND;
		_sensorFault &= ~SENSOR_LAND;
	}
}

int Cei::landMeasure(Vector<float, n_y_land> &y)
{
...
void Cei::landCorrect()
{
	...
	if (beta / BETA_TABLE[n_y_land] > beta_thresh) {
		if (!(_sensorFault & SENSOR_LAND)) {
			_sensorFault |= SENSOR_LAND;
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] land fault,  beta %5.2f", double(beta));
		}
		...
		_sensorFault &= ~SENSOR_LAND;
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] land OK");
	}
...
void Cei::landCheckTimeout()
{
	if (_timeStamp - _time_last_land > LAND_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_LAND)) {
			_sensorTimeout |= SENSOR_LAND;
			_landCount = 0;
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] land timeout ");
		}
	}
}
