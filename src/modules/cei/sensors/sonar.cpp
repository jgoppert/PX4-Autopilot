...
void Cei::sonarInit()
{
	...
	if (_sonarStats.getCount() > REQ_SONAR_INIT_COUNT) {
		if (_sonarStats.getStdDev()(0) > SONAR_MAX_INIT_STD) {
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] sonar init std > min");
			_sonarStats.reset();

		} else if ((_timeStamp - _time_init_sonar) > SONAR_TIMEOUT) {
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] sonar init timeout ");
			_sonarStats.reset();

		} else {
			PX4_INFO("[cei] sonar init "
				 "mean %d cm std %d cm",
...
int Cei::sonarMeasure(Vector<float, n_y_sonar> &y)
{
	// measure
...
void Cei::sonarCorrect()
{
	...
	if (beta > BETA_TABLE[n_y_sonar]) {
		if (!(_sensorFault & SENSOR_SONAR)) {
			_sensorFault |= SENSOR_SONAR;
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] sonar fault,  beta %5.2f", double(beta));
		}
		...
		_sensorFault &= ~SENSOR_SONAR;
		//mavlink_and_console_log_info(&mavlink_log_pub, "[cei] sonar OK");
	}
...
void Cei::sonarCheckTimeout()
{
	if (_timeStamp - _time_last_sonar > SONAR_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_SONAR)) {
			_sensorTimeout |= SENSOR_SONAR;
			_sonarStats.reset();
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] sonar timeout ");
		}
	}
}
