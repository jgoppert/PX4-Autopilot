	...
	if (_gpsStats.getCount() > REQ_GPS_INIT_COUNT) {
		...
		if (!_receivedGps) {
			...
			if (!_map_ref.init_done && !(_fusion.get() & FUSE_VIS_POS)) {
				...
				mavlink_and_console_log_info(&mavlink_log_pub, "[cei] global origin init (gps) : lat %6.2f lon %6.2f alt %5.1f m",
							     gpsLatOrigin, gpsLonOrigin, double(_gpsAltOrigin));
			}

			PX4_INFO("[cei] gps init "
...
int Cei::gpsMeasure(Vector<double, n_y_gps> &y)
{
	// gps measurement
	y.setZero();
...
void Cei::gpsCorrect()
{
	...
	if (beta / BETA_TABLE[n_y_gps] > beta_thresh) {
		if (!(_sensorFault & SENSOR_GPS)) {
			mavlink_log_critical(&mavlink_log_pub, "[cei] gps fault %3g %3g %3g %3g %3g %3g",
		...
		_sensorFault &= ~SENSOR_GPS;
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] GPS OK");
	}
...
void Cei::gpsCheckTimeout()
{
	if (_timeStamp - _time_last_gps > GPS_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_GPS)) {
			_sensorTimeout |= SENSOR_GPS;
			_gpsStats.reset();
			mavlink_log_critical(&mavlink_log_pub, "[cei] GPS timeout ");
		}
	}
}
