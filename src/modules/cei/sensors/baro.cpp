...
int Cei::baroMeasure(Vector<float, n_y_baro> &y)
{
	//measure
	y.setZero();
...
void Cei::baroCorrect()
{
	...
	if (beta > BETA_TABLE[n_y_baro]) {
		if (!(_sensorFault & SENSOR_BARO)) {
			mavlink_log_critical(&mavlink_log_pub, "[cei] baro fault, r %5.2f m, beta %5.2f",
		...
		_sensorFault &= ~SENSOR_BARO;
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] baro OK");
	}
...
void Cei::baroCheckTimeout()
{
	if (_timeStamp - _time_last_baro > BARO_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_BARO)) {
			_sensorTimeout |= SENSOR_BARO;
			_baroStats.reset();
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] baro timeout ");
		}
	}
}
