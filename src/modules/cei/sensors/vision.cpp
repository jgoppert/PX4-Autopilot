...
void Cei::visionInit()
{
	...
	if (_visionStats.getCount() > REQ_VISION_INIT_COUNT) {
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] vision position init: "
		...
		if (!_map_ref.init_done && _sub_vision_pos.get().xy_global) {
			...
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] global origin init (vision) : lat %6.2f lon %6.2f alt %5.1f m",
...
int Cei::visionMeasure(Vector<float, n_y_vision> &y)
{
	y.setZero();
...
void Cei::visionCorrect()
{
	...
	//mavlink_and_console_log_info(&mavlink_log_pub, "[cei] vision delay : %0.2f ms", double(vision_delay * 1000.0f));
	...
	if (beta > BETA_TABLE[n_y_vision]) {
		if (!(_sensorFault & SENSOR_VISION)) {
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] vision position fault, beta %5.2f", double(beta));
			_sensorFault |= SENSOR_VISION;
		}

	} else if (_sensorFault & SENSOR_VISION) {
		_sensorFault &= ~SENSOR_VISION;
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] vision position OK");
	}
...
void Cei::visionCheckTimeout()
{
	if (_timeStamp - _time_last_vision_p > VISION_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_VISION)) {
			_sensorTimeout |= SENSOR_VISION;
			_visionStats.reset();
			mavlink_log_critical(&mavlink_log_pub, "[cei] vision position timeout ");
		}
	}
}
