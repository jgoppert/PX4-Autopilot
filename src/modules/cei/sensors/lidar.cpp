...
void Cei::lidarInit()
{
	...
	if (_lidarStats.getCount() > REQ_LIDAR_INIT_COUNT) {
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] lidar init: "
...
int Cei::lidarMeasure(Vector<float, n_y_lidar> &y)
{
	// measure
...
void Cei::lidarCorrect()
{
	...
	if (beta > BETA_TABLE[n_y_lidar]) {
		if (!(_sensorFault & SENSOR_LIDAR)) {
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] lidar fault,  beta %5.2f", double(beta));
			_sensorFault |= SENSOR_LIDAR;
		}
		...
		_sensorFault &= ~SENSOR_LIDAR;
		mavlink_and_console_log_info(&mavlink_log_pub, "[cei] lidar OK");
	}
...
void Cei::lidarCheckTimeout()
{
	if (_timeStamp - _time_last_lidar > LIDAR_TIMEOUT) {
		if (!(_sensorTimeout & SENSOR_LIDAR)) {
			_sensorTimeout |= SENSOR_LIDAR;
			_lidarStats.reset();
			mavlink_and_console_log_info(&mavlink_log_pub, "[cei] lidar timeout ");
		}
	}
}
