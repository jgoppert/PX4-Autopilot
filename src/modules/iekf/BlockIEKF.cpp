	...
	_x(), _u(), _P()
{
	...
	//"iekf_interval");
	_err_perf = perf_alloc(PC_COUNT, "iekf_err");
...
void BlockIEKF::publishLocalPos()
{
...
void BlockIEKF::publishEstimatorStatus()
{
	if (isfinite(_x(X_x)) && isfinite(_x(X_y)) && isfinite(_x(X_z)) &&
	    isfinite(_x(X_vx)) && isfinite(_x(X_vy))
	    && isfinite(_x(X_vz))) {
...
void BlockIEKF::publishGlobalPos()
{
...
void BlockIEKF::publishFilteredFlow()
{
...
void BlockIEKF::correctGps()  	// TODO : use another other metric for glitch detection
{
