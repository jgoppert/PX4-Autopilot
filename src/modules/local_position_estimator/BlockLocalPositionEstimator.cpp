...
void BlockLocalPositionEstimator::predict()
{
	...
	// note that Q, R are really noise power matrices
	// for continuous time to discrete time conversion:
	//	d/dt(x) = Ax + Bu + w
	// 	y = Cx + Du + v
	// 	E[ww^T] = Q delta(t-tau), where t-tau is a time shift
	// 	E[uu^T] = R delta(t-tau)
	// 	E[vv^T] = R_v delta(t-tau)
	//  	Q_k = Q*dt
	//  	R_k = R*dt
	//  	R_v_k = R_v/dt
	//
	//  Therefore in our euler integration step:
	//  	d/dt(P_k) = AP + PA^T + Q + BRB^T
	//  	P_(k+1) = P_k + (AP + PA^T + Q + BRB^T)*dt
	//              = P_k + (AP + PA^T)*dt + Q_k + BR_kB^T
	//
	//  (see Brown and Hwang 7.1 Introduction to Random Signal Analysis and Kalman Filtering)
}
